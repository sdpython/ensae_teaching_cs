import urllib,urllib2,  os, time, random, codecs
import mechanize
import googlemaps
    ...
    if os.path.exists (file) :
        ...
        if engine == "mappy" :
            ...
            if static_google_maps == None :
                static_google_maps = googlemaps.GoogleMaps (api_key="ABQIAAAA3LCD_QWHgQyYWMUdx1mKtRTOLnapKzarYQ7apQ46S420-wKw2BQiR10GuPti7gX9-PpvG3ttFYn4Uw")
            try :
                x = static_google_maps.directions (v1,v2)
            except googlemaps.GoogleMapsError, e :
                raise Exception ("unable to get direction for " + v1 + "," + v2 +"\n" + str(e))
            r = str(x)
        else :
        ...
        if v1.lower() not in rl or v2.lower() not in rl :
            raise Exception ("unable to find cities from url\n" + url + "\n" + r)
...
if __name__ == "__main__" :
    ...
    villes = [ _.strip("\n\r\t ") for _ in open ("villes.txt", "r").readlines () if len (_) > 2 ]
    villes.sort()
    print villes
