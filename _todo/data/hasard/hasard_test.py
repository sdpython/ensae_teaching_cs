...
chideux_table = [3.84,5.99,7.82,9.49,11.07,12.59,14.07,15.51,16.92,18.31,19.68,21.03]
...
def tirageT (T, fonction, p1, p2, p3) :
    ...
    all = [x1]
    while len(all) < T :
        if      x1 in [1,2] : x2 = fonction(p1)
        elif    x1 in [3,4] : x2 = fonction(p2)
        else                : x2 = fonction(p3)
        all.append(x2)
        x1=x2
    return tuple(all)
...
def tiragesT (T, N, fonction, p1, p2, p3) :
    ...
    for i in xrange (0,N) :
        ...
        if xx[-1] not in cont : cont [xx[-1]] = 1
        else : cont[xx[-1]] += 1
    return cont
