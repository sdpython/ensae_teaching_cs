...
import math                   # pour les racines carr�es


class point (object) :
    """d�finition d'un point : deux coordonn�es et une masse"""
    __slots__ = "x","y","m"
    
    def __init__ (self, x,y,m) :
        """d�finit un point de la corde, de coordonn�es (x,y)
        et de masse m"""
...
class corde (object) :
    """d�finition d'une corde, une liste de points"""
    ...
    def display_courbe_theorique (self, screen) :
        ...
        def asinh (x) :
            """retourne la r�ciproque du sinus hyperbolique de x"""
            t = x + math.sqrt (x*x + 1)
            return math.log (t)
        
        def acosh (x) :
            """retourne la r�ciproque du cosinus hyperbolique de x"""
            t = abs (x) + math.sqrt (x*x - 1)
            return math.log (t)
            
        def trouve_alpha (x2, L) :
            """d�termine la valeur de alpha une seule fois"""
            ...
            while abs (diff) > 1e-3 :
                if diff > 0 : # faire d�cro�tre alpha
        ...
        if p1.y != p2.y : return None # cas non pr�vu
