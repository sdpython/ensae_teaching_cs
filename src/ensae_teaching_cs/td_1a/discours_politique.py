...
import re, os, json, io, html.parser
...
def xmlParsingLongestDiv (text) :
    ...
    global endLine
    res = best.replace ("<p>","").replace("</p>",endLine).replace("\r","").replace ("<br />",endLine).replace ("<br>",endLine)
    exp = re.compile("[|]((.|\n){5,50}) ")
    nb = exp.findall(res)
...
def force_unicode (text) :
    ...
    while True :
        try :
            ...
            text = text.replace("  "," ").replace("â€™","'").replace("a§","c")
            text = text.replace("a¹","u").replace("a¨","e").replace("a‰","E")
            text = text.replace("a¢","a").replace("aª","e").replace("aƒÂ´","o")
            text = text.replace("aƒÂ©","e").replace("aƒÂ","e").replace("Â©","e")
            text = text.replace("a»","u").replace("â‚¬","E").replace("a®","i")
            text = text.replace('\xa0', " ").replace("Å“","oe").replace("Â«"," ")
            text = text.replace ("Â»"," ").replace("e¹ ","ei").replace("‚Â", " ")
            turn+=1
            if turn>100 :
                # too much
                return None
...
def retrieve_speeches_json(url = "http://www.elysee.fr/chronologie/download/json"):
    """
...
def get_elysee_speech_from_elysees(title, url = "http://www.elysee.fr/chronologie/article/"):
    """
    ...
    if title.startswith("http"):
        ...
        link = remove_accent(title.lower()).replace(" ","-").replace("'","-").replace('"',"")
        full = url + "/" + link + "/"
    try:
