...
class TestCorde(unittest.TestCase):
    ...
    def test_image_video_pendule(self):
        fLOG(
            __file__,
            self._testMethodName,
            OutputPrint=__name__ == "__main__")
        temp = get_temp_folder(__file__, "temp_image_video_pendule")
        ...
        pygame_simulation(pygame, fLOG=fLOG,
                          iter=4000 if __name__ == "__main__" else 100,
                          folder=temp, flags=flags, model="pendule")
        files = os.listdir(temp)
        self.assertTrue(len(files) > 9)
        png = [os.path.join(temp, _)
               for _ in files if os.path.splitext(_)[-1] == ".png"]
        self.assertTrue(len(png) > 0)
        out = os.path.join(temp, "pendule.avi")
